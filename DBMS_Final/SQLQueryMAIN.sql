...
(
    PhongBan_MaPB INT IDENTITY(1,1) PRIMARY KEY,
...
GO
-- Bảng HeSoLuong
CREATE TRIGGER trg_HeSoLuong_ID
ON HeSoLuong
AFTER INSERT
AS
BEGIN
    DECLARE @max_id INT;
    SELECT @max_id = MAX(HeSoLuong_ID) FROM HeSoLuong;
    UPDATE HeSoLuong SET HeSoLuong_ID = @max_id + 1 WHERE HeSoLuong_ID IN (SELECT HeSoLuong_ID FROM inserted);
END;
GO
...
BEGIN
    BEGIN TRY
        INSERT INTO HeSoLuong (HeSoLuong_Ten, HeSoLuong_GiaTri) VALUES (@HeSoLuong_Ten, @HeSoLuong_GiaTri);
...
BEGIN
    ...
    END
        ...
        If(@PhanQuyen = 0) -- nhan vien
            ...
            IF(@Loai = 1)
                    BEGIN
                -- NhaVien, HopDong, HSL, PhongBan, ChucVu
                SET @sqlUser = N'GRANT EXECUTE ON sp_ThemMoiChucVu TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON sp_XoaChucVu TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON sp_SuaChucVu TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                -- PhongBan
                SET @sqlUser = N'GRANT EXECUTE ON sp_ThemPhongBan TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON sp_XoaPhongBan TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON sp_CapNhatPhongBan TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON ThemNhanVien TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON XoaNhanVien TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON CapNhatNhanVien TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON ThemHopDong TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON SuaHopDong TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
                SET @sqlUser = N'GRANT EXECUTE ON XoaHopDong TO ' + QUOTENAME(@SoTK) + N';';
                EXEC sp_executesql @sqlUser;
            END
                    ELSE
                    BEGIN
                IF(@Loai = 0)
                    ...
                    SET @sqlUser = N'GRANT SELECT ON DS_KyCong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON ThemKyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON CapNhatKyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON CapNhatTrangThaiKyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON ThongKeCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON XoaKCCT TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON PhatSinhKyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON XoaChamCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON CapNhatChamCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON Find_MaKyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON KT_PhatSinhKyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON KT_NgayTrongTuan TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON SoNgayTrongThang TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON SoNgayCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON TK_KyCong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON ungluongtheoid TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON LayThongtinUngLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON ThemUngLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON SuaUngLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON XoaUngLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON LuuXoaUngLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON HuyXoaUngLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    -- -- Tangca
                    SET @sqlUser = N'GRANT EXECUTE ON LayThongtinTangCa TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON SuaTangCa TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON tangcatheoid TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON ThemTangCa TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON XoaTangCa TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    --Luong
                    SET @sqlUser = N'GRANT EXECUTE ON TinhLuong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON hienthiluong TO ' + QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON xoaluongtheokycong TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    --LoaiTangCa
                    SET @sqlUser = N'GRANT EXECUTE ON sp_ThemMoiLoaiTangCa TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON sp_XoaLoaiTangCa TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                    SET @sqlUser = N'GRANT EXECUTE ON sp_SuaLoaiTangCa TO '+ QUOTENAME(@SoTK) + N';';
                    EXEC sp_executesql @sqlUser;
                END
            END
        END
...
EXEC ThemTaiKhoan '20110110', '123', 1, 0
GO
EXEC ThemTaiKhoan '20110111', '123', 0, 1
GO
EXEC ThemTaiKhoan '20110112', '123', 0, 0
GO

--Tạo tài khoản mặc định với quyền như 'sa'
CREATE LOGIN [20110] WITH PASSWORD = 'admin', CHECK_POLICY = OFF
GO
USE QLNS
GO
CREATE USER [20110] FOR LOGIN [20110]
GO
ALTER SERVER ROLE sysadmin ADD MEMBER [20110]
GO
...
BEGIN
    ...
    END TRY
    BEGIN CATCH;
        THROW 51000, 'Lỗi khi xoá nhân viên!',1;
    END CATCH;
END
GO
--Test xoá nhân viên
DECLARE @Message NVARCHAR(100);
 EXECUTE XoaNhanVien @NhanVienID = 222, @Message = @Message OUTPUT;
PRINT @Message;
...
DECLARE @Message NVARCHAR(100);
 EXECUTE CapNhatNhanVien 
    @NhanVienID = 18,
    @HoTen = N'Huỳnh Thị My',
    @SDT = '0312179124',
    @CCCD = '049002113233',
     @GioiTinh = N'Nữ',
    @HinhAnh = NULL,
    @DiaChi = N'122 đường số 9, TPHCM',
     @NgaySinh = '1990-01-01',
     @ChucVu = 3,
    @PhongBan = null,
    @Message = @Message OUTPUT;
...
-- EXEC TimKiemNhanVien @Ten = N'Nguyễn';
